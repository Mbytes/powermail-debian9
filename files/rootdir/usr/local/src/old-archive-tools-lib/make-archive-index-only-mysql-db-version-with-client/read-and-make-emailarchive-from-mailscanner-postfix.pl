...
my $myfilelist2="/tmp/mail-for-process-for-emailarchive-final-list2";
my $cmx="find /mail-archive-uncompress/ -type f > ".$myfilelist;
`$cmx`;
my $loc = DBI->connect("dbi:mysql:server=localhost;database=emailarchive;host=localhost",'emailarchive','rI7v6mLiuoAnAoTD');
...
my $cmx="find /mail-archive-process/ -type f > ".$myfilelist2;
`$cmx`;
...
my $cmdxpost="postcat -hb $directeml  > /mail-archive-process/".$newtmpfile." ";
...
my $mkfolderforemail="/mail-archive-compress/".$emailyear."/".$emaildate."/".$emailhour."-".$emailmin."/".$emailsec."/";
