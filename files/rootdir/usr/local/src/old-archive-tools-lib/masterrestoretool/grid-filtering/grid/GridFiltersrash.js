...
if(i!=0)
{
masterkeyx=masterkeyx+"&";
}

var f    = filters[i];
                        var root = [this.paramPrefix, '[', i, ']'].join('');
                        pa[root + '[field]'] = f.field;
  //                    alert(root);
                        var dataPrefix = root + '[data]';
    //                    alert(dataPrefix);
masterkeyx=masterkeyx+root+'[field]='+f.field;
for(var key in f.data)
{
masterkeyx=masterkeyx+"&";
masterkeyx=masterkeyx+root+'[data]['+key+']='+f.data[key];
   pa[[dataPrefix, '[', key, ']'].join('')] = f.data[key];
}
//alert(f.field+">>2");
//alert(f.data[key]+">>3");
//alert(f.data.comparison+">>4");
